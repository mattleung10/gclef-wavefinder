					   ...
					   TW_MEMREF pData)
{
	if(IsValidDriver())
	{
		...
		/*
		* JTZ: Modification2: For <DG_CONTROL,DAT_EVENT,MSG_PROCESSEVENT>, the
		* return value is either TWRC_DSEVENT or TWRC_NOTDSEVENT, there's no 
		* need to call <DG_CONTROL,DAT_STATUS,MSG_GET> anymore.. and actually,
		* calling it will cause problem.
		* As m_Status is NOT used anywhere, we simply remove those code below.
		*/
		/*
...
BOOL CTwain::ProcessMessage(MSG msg)
{
	if(SourceEnabled())
	{
		...
		if(GetRC() != TWRC_NOTDSEVENT) // Yes, this is the DS Msg...
		{
			TranslateMessage(twEvent);
			/*
			* JTZ: Modification1: For TWRC_DSEVENT, we should return TRUE back to inform
			* the caller that it should bypass the ::TranslateMessage() and ::ProcessMessage(),
			* As the messages are already processed by the TWAIN DS. This is stated clearly
			* on TWAIN Spec.
			*/
			return TRUE;
		}
